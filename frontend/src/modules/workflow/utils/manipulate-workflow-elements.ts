...
/**
 * Update a JSON attribute on a workflow element by ID
 * @param workflowText - The complete workflow XML text
 * @param elementId - The ID of the element to update
 * @param attributeName - The name of the attribute to update (e.g., 'data', 'config', 'props')
 * @param jsonData - The JSON data to set (will be stringified)
 * @returns Updated workflow text with the attribute modified
 */
export function updateElementJsonAttribute(
    workflowText: string,
    elementId: string,
    attributeName: string,
    jsonData: Record<string, any>
): string {
    console.log('[updateElementJsonAttribute] Updating element:', {
        elementId,
        attributeName,
        jsonData,
    });
    ...
    try {
        ...
        const parserError = doc.querySelector('parsererror');
        if (parserError) {
            console.error('[updateElementJsonAttribute] XML parsing error:', parserError.textContent);
            return workflowText;
        }
        ...
        const element = findElementById(doc, elementId);
        if (!element) {
            console.error('[updateElementJsonAttribute] Element not found:', elementId);
            return workflowText;
        }
        ...
        console.error('[updateElementJsonAttribute] Error:', error);
        return workflowText;
    }
}

/**
 * Get a JSON attribute from a workflow element by ID
 * @param workflowText - The complete workflow XML text
 * @param elementId - The ID of the element to read from
 * @param attributeName - The name of the attribute to read (e.g., 'data', 'config', 'props')
 * @returns Parsed JSON data or null if not found
 */
export function getElementJsonAttribute(
    workflowText: string,
    elementId: string,
    attributeName: string
): Record<string, any> | null {
    console.log('[getElementJsonAttribute] Reading element:', {
        elementId,
        attributeName,
    });
    ...
    try {
        ...
        const parserError = doc.querySelector('parsererror');
        if (parserError) {
            console.error('[getElementJsonAttribute] XML parsing error:', parserError.textContent);
            return null;
        }
        ...
        const element = findElementById(doc, elementId);
        if (!element) {
            console.error('[getElementJsonAttribute] Element not found:', elementId);
            return null;
        }
        ...
        if (!attributeValue) {
            console.warn('[getElementJsonAttribute] Attribute not found:', attributeName);
            return null;
        }
        ...
        console.error('[getElementJsonAttribute] Error:', error);
        return null;
    }
}
